...
new class extends Component
{
    ...
    public function mount($id)
    {
        ...
        $this->event = DisasterEvent::find($barang->disaster_event_id);

        $selected = session('admin_disaster_event_id');
        if ($selected && (int) $selected !== (int) $barang->disaster_event_id) {
            session()->flash('error', 'Data ini milik event berbeda. Silakan pilih event yang sesuai di filter.');
    ...
    public function update()
    {
        ...
        if (!$event || $event->status !== 'active') {
            session()->flash('error', 'Tidak bisa mengubah data karena event sudah diarsipkan atau tidak ditemukan.');
            return;
        }
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <form wire:submit="update" class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input wire:model="nama_barang" type="text" id="nama_barang" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-mdmc-600 focus:border-mdmc-600 transition-all outline-none">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <select wire:model="jenis_id" id="jenis_id" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-mdmc-600 focus:border-mdmc-600 transition-all outline-none bg-white">
