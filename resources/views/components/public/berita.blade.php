...
<div class="min-h-screen bg-slate-50">
    ...
    <nav class="bg-white/90 backdrop-blur-md sticky top-0 z-40 border-b border-slate-100">
        <div class="max-w-7xl mx-auto px-6 lg:px-12 h-20 flex items-center justify-between" x-data="{ open: false }">
            ...
            <div class="hidden lg:flex items-center gap-8">
                ...
                <a href="/berita" class="text-slate-900 font-extrabold text-sm">Berita</a>
            ...
            <div x-show="open" x-cloak class="absolute top-20 left-0 right-0 bg-white border-b border-slate-100 lg:hidden">
                <div class="px-6 py-4 space-y-2">
                    ...
                    <a href="/berita" class="block px-3 py-2 rounded-xl font-bold text-slate-900 bg-slate-50">Berita</a>
    ...
    <section class="px-6 lg:px-12 py-14 bg-white border-b border-slate-100">
        <div class="max-w-7xl mx-auto">
            <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">Berita</p>
        ...
        <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10">
            <div class="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-6">
                @foreach($posts as $post)
                    <a href="#" class="group bg-white rounded-3xl border border-slate-100 overflow-hidden hover:shadow-lg transition-all">
                        ...
                        <div class="p-6">
                            <div class="flex items-center gap-2 text-xs">
                                <span class="bg-slate-50 text-slate-700 px-2.5 py-1 rounded-full font-bold">{{ $post['category'] ?? 'Berita' }}</span>
                                <span class="text-slate-400 font-bold">{{ $post['date'] ?? '' }}</span>
                            </div>
                            <h2 class="mt-3 text-lg font-extrabold text-slate-900 leading-snug group-hover:text-blue-600 transition-colors">{{ $post['title'] ?? '-' }}</h2>
                        </div>
                    </a>
                @endforeach
            </div>
            ...
            <aside class="lg:col-span-4 space-y-6">
                ...
                <div class="bg-slate-900 text-white rounded-3xl p-6">
                    ...
                    <p class="mt-2 text-sm text-slate-300">Jika Anda berada di area terdampak bencana, gunakan form resmi untuk permintaan bantuan.</p>
                    <div class="mt-5">
                        @if($emergencyMode)
