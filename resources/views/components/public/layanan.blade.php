...
<div class="min-h-screen bg-[#F6F9FF]">
    <nav class="bg-gradient-to-b from-[#0B1B43]/95 via-[#162E67]/95 to-[#233876]/95 backdrop-blur-md sticky top-0 z-50 border-b border-white/10">
        <div class="max-w-[1280px] mx-auto px-6 lg:px-12 h-20 flex items-center justify-between" x-data="{ open: false }">
            ...
            <div class="hidden lg:flex items-center gap-7">
                ...
                <a href="/layanan" class="text-white font-extrabold text-sm border-b-2 border-white pb-1">Layanan</a>
            ...
            <div x-show="open" x-cloak class="absolute top-20 left-0 right-0 bg-[#233876] border-b border-white/10 lg:hidden">
                <div class="px-6 py-4 space-y-2">
                    ...
                    <a href="/layanan" class="block px-3 py-2 rounded-xl font-bold text-white bg-white/10">Layanan</a>
    ...
    <header class="bg-gradient-to-br from-[#142B63] via-[#233876] to-[#142B63]">
        <div class="max-w-[1280px] mx-auto px-6 lg:px-12 py-10">
            <p class="text-blue-100/80 text-xs font-extrabold uppercase tracking-widest">Layanan</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    @foreach($cards as $card)
                        <div class="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm">
                            <p class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">{{ $card['label'] ?? 'Layanan' }}</p>
                            <p class="mt-3 text-lg font-extrabold text-slate-900">{{ $card['title'] ?? '-' }}</p>
                            <p class="mt-2 text-slate-500 font-semibold">{{ $card['desc'] ?? '' }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <aside class="lg:col-span-4 space-y-6">
                <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                    <div class="p-6 border-b border-slate-100">
                        ...
                        <p class="mt-2 text-xl font-extrabold text-slate-900">Video Mitigasi</p>
                        <p class="mt-2 text-sm text-slate-500 font-semibold">Highlight video untuk edukasi kesiapsiagaan.</p>
                    </div>
                ...
                <div class="bg-gradient-to-br from-[#142B63] via-[#233876] to-[#142B63] text-white rounded-3xl p-6 shadow-[0_20px_60px_rgba(35,56,118,0.22)]">
                    ...
                    <p class="mt-2 text-sm text-blue-100/90 font-semibold">Jika mode bencana aktif, Anda bisa mengirim permintaan bantuan melalui form resmi.</p>
                    <div class="mt-5">
                        @if($emergencyMode)
